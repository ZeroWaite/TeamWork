...
import com.example.administrator.teamwork.MyInfo.InterUsersInfo;
...
public class FragUsers extends Fragment {
            ...
            switch (msg.what) {
                case MSG:
                    ...
                    if (prettyGirlAdapter == null) {
                        prettyGirlAdapter = new ImgListAdapter(mList, FragUsers.this.getActivity(),4);
                    } else {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.search_users_list, null);
        return view;
    }
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        recyclerView = (RecyclerView) view.findViewById(R.id.rv_users_list);


        Intent intent = getActivity().getIntent();
        choice = intent.getExtras().getString("choice");
        getUrl = "http://api.huaban.com/users/favorite/"+choice ;
    ...
    public void inteData() {
        demo_swiperefreshlayout = (SwipeRefreshLayout)FragUsers.this.getActivity().findViewById(R.id.demo_users_list);
        //设置刷新时动画的颜色，可以设置4个
    ...
    private List<LocalShareInfo> getJsonData(String str) {
        ...
        Gson gson = new Gson();
        interBoardInfo = gson.fromJson(str, InterUsersInfo.class);
        for (int i = 0; i < interBoardInfo.getPusers().toArray().length; i++) {
            localPrettyGirlInfo = new LocalShareInfo();
            localPrettyGirlInfo.setUsername(interBoardInfo.getPusers().get(i).getUser().getUsername());
            localPrettyGirlInfo.setUserHead(HTTP + interBoardInfo.getPusers().get(i).getUser().getAvatar().getKey());
            localPrettyGirlInfo.setFollow_count(interBoardInfo.getPusers().get(i).getUser().getFollower_count() == 0 ? "0" :
                    String.valueOf(interBoardInfo.getPusers().get(i).getUser().getFollower_count())+"粉丝");
